                ...
                <div class="my-md-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-3 flex-nowrap flex-xl-wrap overflow-auto overflow-xl-visble">
                            ...
                            @php
                            $catename = \App\Models\Category::find($product->category->id);
                            $subcatename = \App\Models\SubCategory::find($product->subcategory->id);
                        @endphp
                            
                                <li class="breadcrumb-item flex-shrink-0 flex-xl-shrink-1"><a
                                        href="{{ route('products.by.category', $catename->slug) }}">
                                        {{ $catename->name }}</a></li>
                           
                                <li class="breadcrumb-item flex-shrink-0 flex-xl-shrink-1"><a
                                        href="{{ route('products.by.subcategory', $subcatename->slug) }}">
                                        {{ $subcatename->name }}</a></li>
                ...
                <div class="row">
                    <div class="col-md-6 col-lg-4 col-xl-5 mb-4 mb-md-0">
                        <div id="sliderSyncingNav" class="js-slick-carousel u-slick mb-2" data-infinite="true"
                            ...
                            <div class="js-slide">
                                <img class="img-fluid" src="{{ $product->image }}" alt="Image Description">
                            </div>

                            @foreach ($product->productimages as $images)
                                @php
                                    $gallery_images = explode(',', $images->images);
                                @endphp
                                ...
                                @foreach ($gallery_images as $gallery)
                                    @if ($gallery)
                                        <div class="js-slide">
                                            <img class="img-fluid"
                                                src="{{ asset('assets/images/products') }}/{{ $gallery }}"
                            ...
                            <div class="card p-5 border-width-2 border-color-1 borders-radius-17">
                                ...
                                <div class="mb-3">
                                    <div class="prodcut-price d-flex align-items-center flex-wrap position-relative"
                                        style="top:10px">
                                        <ins class="font-size-20 text-red text-decoration-none mr-2 price_attribute"
                                            data-price="{{ $product->sale_price }}">Rs.{{ $product->sale_price }}</ins>
                                        <ins class="font-size-20 text-red text-decoration-none mr-2 price_change"
                                            style="display: none"></ins>
                                        <del
                                            class="font-size-12 tex-gray-6 position-absolute bottom-100">Rs.{{ $product->regular_price }}</del>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <div class="border rounded-pill py-1 w-md-60 height-35 px-3 border-color-1">
                                        <div class="js-quantity row align-items-center">
                                            <div class="col">
                                                <input
                                                    class="js-result form-control h-auto border-0 rounded p-0 shadow-none quantity"
                                                    type="text" value="1">
                                            </div>
                                ...
                                <div class="mb-3">
                                    <h6 class="font-size-14">Extra Specifications</h6>
                                    <!-- Select -->
                                    <div class="dropdown bootstrap-select js-select dropdown-select btn-block col-12 px-0">

                                        <select
                                            class="js-select selectpicker dropdown-select btn-block col-12 px-0 variation_id"
                                            data-style="btn-sm bg-white font-weight-normal py-2 border" tabindex="-98"
                                            id="specs_id">
                                            <option value="">No Selected</option>
                                            @foreach ($product->variations as $variation)
                                                <option value="{{ $variation->id }}"
                                                    data-price="{{ $variation->price }}">{{ $variation->attribute }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="mb-2 pb-0dot5">
                                    <a href="javascript:;" class="btn btn-block btn-primary-dark add_to_cart"
                                        data-product_id="{{ $product->id }}" data-price="{{ $product->sale_price }}"><i
                                ...
                                <div class="product-item__inner px-xl-4 p-3">
                                    <div class="product-item__body pb-xl-2">
                                        ...
                                        <h5 class="mb-1 product-item__title"><a
                                                ...
                                                class="text-blue font-weight-bold">{{ $productItem->name }}</a></h5>
                                        <div class="mb-2">
                                            <a href="{{ route('product.details', $productItem->slug) }}"
                                                class="d-block text-center"><img class="img-fluid"
                                                    src="{{ $productItem->image }}" alt="Image Description"></a>
                                        </div>
                                    ...
                                    <div class="product-item__footer">
                                        <div class="border-top pt-2 flex-center-between flex-wrap">
                                            ...
                                            @else
                                                <a href="../shop/wishlist.html" class="text-gray-6 font-size-13"><i
                                                        class="ec ec-add-to-cart mr-1 font-size-15"></i> Select Options</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
...
@section('custom-scripts')
    <script>
        $(document).on('change', '#specs_id', function() {
            var initial_price = $('ins.price_attribute').data('price');
            var price = $(this).find('option:selected').data('price');
            var id = $(this).val();
            if (id) {
                price = parseInt(price) + parseInt(initial_price);
                $('.price_change').text('Rs.' + price);
                $('.price_attribute').css('display', 'none');
                $('.price_change').css('display', 'block');

            } else {
                $('.price_attribute').css('display', 'block');
                $('.price_change').css('display', 'none');
